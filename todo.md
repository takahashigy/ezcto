...
## 添加合约地址（CA）功能
- [x] 在LaunchV2表单添加CA输入框（可选字段）
- [x] 在projects表添加contractAddress字段
- [x] 修改AI生成逻辑，在网站显眼位置展示CA
- [x] 添加CA一键复制功能（点击复制到剪贴板）
- [ ] 测试CA显示和复制功能

## 修夏LaunchV2图片src错误
- [x] 修夏LaunchV2页面中图片src为空字符串的错误（当imagePreview为空时不渲染img标签）

## 修夏Nanobanana API调用失败问题
- [x] 检查Nanobanana API配置和环境变量
- [x] 添加详细错误日志（包含完整的错误信息和请求参数）
- [x] 优化重试策略（增加超时时间到120秒）
- [x] 改善用户反馈（每张图生成时显示进度 X/6）
- [ ] 测试修复后的生成流程

## 并行图片生成优化
- [x] 修改launch.ts将图片生成从串行改为并行（使用Promise.all）
- [x] 优化并行生成的进度反馈（显示已完成数量）
- [x] 确保每张图生成后立即保存到数据库
- [x] 修复并发竞争条件（使用Set跟踪已完成图片）
- [ ] 测试并行生成流程（总时间应从6分钟缩短到1分钟）

## 修复免费配额限制错误
- [x] 查找projects.create中的免费配额检查逻辑
- [x] 移除或放宽免费配额限制（允许管理员或测试用户创建多个项目）
- [ ] 测试修复后的创建流程
...
## 独立部署问题诊断
- [ ] 检查当前环境配置和API调用逻辑
- [ ] 对比Manus预览环境和独立部署环境的差异
- [ ] 诊断为什么独立运行时API调用失败
- [ ] 提供针对性解决方案（环境变量配置、CORS、域名等）

## 代码清理和Launch V2统一
- [x] 全局检查代码质量和优化点
- [x] 删除Launch(Old)页面文件
- [x] 将所有指向/launch的链接统一为Launch V2
- [x] 更新导航栏移除"Launch (Old)"按钮
- [x] Launch V2页面显示正常，表单完整
...
## 视觉设计系统统一优化
- [x] 分析当前设计系统（背景色、主色、按钮样式）
- [x] 定义统一的设计规范（按钮形状、颜色、间距、字体）
- [x] 优化Home页面按钮和布局
- [x] 优化Launch页面表单和按钮
- [x] 优化Dashboard页面卡片和按钮
- [x] 统一所有页面的视觉风格
- [ ] 测试各页面的视觉一致性
- [ ] 保存checkpoint
...
## 加密货币预付费系统（方案B：先付费后生成）
- [x] 创建支付配置文件（BSC/Solana收款地址、代币合约地址）
- [x] 实现代币持有量检测（0xf036...7777代币 ≥ 100 USD → 7折）
- [x] 创建支付弹窗UI组件（显示两个支付选项、价格、收款地址、二维码）
- [x] 实现BSC链支付验证逻辑（USDT转账监听）
- [x] 实现Solana链支付验证逻辑（USDC转账监听）
- [x] 集成到LaunchV2表单（提交前检查付费状态）
...
## 发布同步问题
- [x] 排查已发布网页与预览网页不一致的问题
- [x] 设置 BSC 为默认链


## 钱包连接后 UI 样式优化
- [x] 设计已连接状态的 UI（地址缩略、链图标、余额）
- [x] 设计下拉菜单（断开连接、切换网络）
- [x] 保持与网站绿色科技风格一致


## 管理员权限和白名单系统（已完成）
- [x] Owner 自动成为 Admin（基于 OWNER_OPEN_ID）
- [x] 白名单数据库表（whitelist 表）
- [x] 白名单管理 API（添加、批量导入、删除、检查状态、使用次数）
- [x] 管理后台页面（/admin/whitelist）
...
## "最新优先"策略实施 (2026-02-03)
### 问题描述
- 用户支付/使用白名单后生成失败
- 用户重新在 Launch 提交时报错，必须删除旧项目
- 用户体验差，支付/白名单可能被浪费

### 解决方案
- [x] 分析现有项目状态和支付逻辑
- [x] 修改后端：检测未完成项目时自动标记为"已替代" (superseded)
- [x] 修改后端：将旧项目的支付记录转移到新项目
- [x] 添加 getIncompleteProjectsByUserId 和 supersedeProject 函数
- [x] 修改前端：移除阻塞报错，允许重新提交
  - 前端错误处理已是通用的，后端不再抛出 FREE_QUOTA_EXCEEDED 错误
  - 后端会自动处理项目替代和支付转移
- [x] 测试并保存 checkpoint

## Launch 页面体验优化 (2026-02-03)
- [x] 移除钱包状态区域的"需要支付 $199 才能生成"提示
- [x] 调整按钮流程：先显示支付界面，最后一步再检查表单
