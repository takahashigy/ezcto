...
export async function analyzeProjectInput(input: {
  ...
  posterPrompt: string;
  // Poster slogan - unique tagline extracted from project narrative for poster
  posterSlogan: string;
...
Please provide your analysis in the following JSON format:
{
  ...
  "posterSlogan": "A unique, powerful slogan extracted from the project narrative for the poster. Requirements: 1) MUST be different from websiteContent.headline and websiteContent.tagline 2) Should be shorter, punchier, more impactful - suitable for visual design 3) Can be a single word, a short phrase, or a brief declaration 4) Must capture the essence of the project's story or mission 5) Style must match the brand personality. Examples: 'Born to Moon', 'The People's Coin', 'Chaos is a Ladder', 'We Are All Going to Make It'",
  "posterPrompt": "Detailed prompt for 1080x1350 magazine-quality editorial poster. CRITICAL REQUIREMENTS: 1) STYLE CONSISTENCY IS THE HIGHEST PRIORITY - strictly follow the established visual style from brandStrategy 2) The ONLY text on the poster MUST be the posterSlogan you generated above - DO NOT invent magazine names, issue numbers, or any other text 3) The ticker ${input.ticker} can appear as a subtle design element but posterSlogan is the main text 4) Editorial portrait style - treat the project mascot/character as the subject of a fashion shoot 5) Typography-driven composition with the posterSlogan as the visual anchor 6) Print-first, CMYK-aware colors using brand palette 7) Strong negative space or controlled density 8) NO generic magazine template text like 'DESIGN FORM' or 'ISSUE XX' - this is STRICTLY FORBIDDEN 9) The poster must feel like it belongs to the same visual universe as hero background and community scene.",
  "posterSectionContent": {
